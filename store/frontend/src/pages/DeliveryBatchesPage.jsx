...
export default function DeliveryBatchesPage() {
  ...
  const handleCreateBatch = async (e) => {
    ...
    try {
      ...
      navigate(`/delivery-batches/${newBatch.id}`);
    } catch (err) {
      console.error(err);
    ...
    <div className="page-container">
      ...
      <div className="grid" style={{ gridTemplateColumns: '3fr 2fr', gap: 'var(--space-lg)', alignItems: 'start' }}>
        ...
        <div className="card">
          ...
          {batches.length === 0 ? (
            <div className="text-center p-lg text-secondary">Chưa có chuyến giao hàng nào được tạo.</div>
          ) : (
                ...
                <tbody>
                  {batches.map((batch) => (
                    <tr key={batch.id}>
                      ...
                      <td>
                        <Link to={`/delivery-batches/${batch.id}`} className="btn btn-secondary p-sm">
                          Chi tiết
                        </Link>
                      </td>
                    </tr>
